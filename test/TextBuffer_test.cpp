...
// Implementation of EditorException and TextBufferException
...
bool TextBuffer::saveToFile(const std::string& filename) const {
    ...
    if (!outfile.is_open()) {
        ai_editor::ERROR_REPORTER.error("Could not open file for saving: " + filename, "TextBuffer::saveToFile");
        return false;
    }
    ...
    if (outfile.fail()) {
        ai_editor::ERROR_REPORTER.error("Failed while writing to file: " + filename, "TextBuffer::saveToFile");
...
bool TextBuffer::loadFromFile(const std::string& filename) {
    ...
    if (!infile.is_open()) {
        ai_editor::ERROR_REPORTER.error("Could not open file for loading: " + filename, "TextBuffer::loadFromFile");
        return false;
    }
